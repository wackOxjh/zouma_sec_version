...
int device_init(struct ls_i2c_dev *dev)
{
    ...
    // 获取陀螺仪 ID
    if (lq_i2c_mpu6050_get_id(dev) == MPU6050_DEV_ADDR) {
        // 初始化MPU6050设备
        if (lq_i2c_mpu6050_init(dev) != 0) {
            return -1;
        }
        printk("%s: init success, addr=0x%x\n", DEVICE_NAME, MPU6050_DEV_ADDR);
    }
    return 0;
}
...
int device_inspection(struct ls_i2c_dev *dev, DeviceInitFunc deviceInit)
{
    ...
    // 重新扫描所有设备
    ret = lq_i2c_mpu6050_get_id(dev);
    if (ret != MPU6050_DEV_ADDR) {
        is_init = false;
