...
int device_init(struct ls_i2c_dev *dev)
{
    ...
    // 初始化vl53l0x设备
    if (lq_i2c_vl53l0x_init(dev) != 0) {
        printk(KERN_ERR "%s: init failed, addr=0x%x\n", DEVICE_NAME, VL53L0X_DEV_ADDR);
        return -1;
    }
...
int device_inspection(struct ls_i2c_dev *dev, DeviceInitFunc deviceInit)
{
    ...
    // 读取设备寄存器检测设备是否存在
    ret = i2c_read_regs(dev, VL53L0X_REG_IDENTIFICATION_MODEL_ID, &val, 1);
    if (ret != 0) {
        is_init = false;
        printk(KERN_ERR "%s: failed to read device\n", DEVICE_NAME);
        return ret;
    }
    if (!is_init) {
        deviceInit(dev);
...
{
    // 从 inode 结构体中获取 cdev，再获取 ls_i2c_dev 结构体
    struct ls_i2c_dev *dev = container_of(inode->i_cdev, struct ls_i2c_dev, cdev);
    // 将设备结构体保存到 file->private_data 中，供其他函数使用
    f->private_data = dev;
    // 初始化设备
    device_init(dev);
    return 0;
...
{
    uint16_t dis = 0;       // 存放读取到的距离值
    // 从 file->private_data 获取设备结构体
    struct ls_i2c_dev *dev = (struct ls_i2c_dev *)f->private_data;
    // 参数检查
    if (unlikely(dev == NULL)) {
        pr_err("%s: i2c_ioctl dev is null\n", DEVICE_NAME);
        return -EINVAL;
    }
    // 根据命令码执行相应的操作
    switch (cmd)
    {
        /**************************************** VL53L0X ****************************************/
        case I2C_GET_VL53L0X_DIS:  // 获取VL53L0X的温度
            vl53l0x_get_distance(dev, &dis);
            if (copy_to_user((int16_t*)arg, &dis, sizeof(uint16_t)))
                return -EFAULT;
            break;
        default:
            return -EFAULT;
...
int i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("%s: i2c_probe called, client addr=0x%02x\n", DEVICE_NAME, client->addr);
    ...
    // 开启定时器
    {
        data->wq = create_workqueue("vl53l0x_cycle_wq");
        if (!data->wq) {
